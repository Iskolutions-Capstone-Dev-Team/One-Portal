...
func TestRefreshTokenRepositoryCRUD(t *testing.T) {
	...
	if err := repo.CreateToken(ctx, rt); err != nil {
	...
	saved, err := repo.GetToken(ctx, rt.Token)
	if err != nil {
	...
	if err := repo.UpdateToken(ctx, rt.Token, newExpiry); err != nil {
	...
	if err := repo.DeleteToken(ctx, rt.Token); err != nil {
	...
	count, err := repo.DeleteExpiredTokens(ctx, now)
	if err != nil {
		t.Fatalf("DeleteExpiredTokens failed: %v", err)
	}
	if count != 3 {
		t.Fatalf("DeleteExpiredTokens rows=%d want=3", count)
	}
