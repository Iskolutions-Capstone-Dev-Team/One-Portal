        ...
        <header className="absolute top-0 left-0 w-full z-50 bg-transparent">
            <div className="flex items-center justify-between px-4 md:px-8 max-w-[1400px] mx-auto h-20 md:h-24">
                ...
                <div className="relative flex items-center gap-3">
                    <Button variant="outline" size="icon" className="w-10 h-10 rounded-xl bg-white/10 hover:bg-white/20 border-white/20 text-white hover:text-white focus:text-white dark:bg-[#f8d24e]/10 dark:hover:bg-[#f8d24e]/20 dark:border-[#f8d24e]/20 dark:text-[#ffe28a] dark:hover:text-[#ffe28a] dark:focus:text-[#ffe28a] transition-colors" onClick={handleThemeToggle} aria-label={themeLabel}>
                        <SunIcon className={cn(
                            "size-5 !text-white dark:!text-[#ffe28a] transition-all duration-300",
                            isDarkMode
                                ? "scale-0 -rotate-90 opacity-0"
                                : "scale-100 rotate-0 opacity-100"
                        )} />
                        <MoonIcon className={cn(
                            "absolute size-5 !text-white dark:!text-[#ffe28a] transition-all duration-300",
                            isDarkMode
                                ? "scale-100 rotate-0 opacity-100"
                                : "scale-0 rotate-90 opacity-0"
                        )} />
                    ...
                    <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
                        <DropdownMenuTrigger asChild>
                            <Button size="icon" variant="outline" className={cn("w-10 h-10 rounded-xl transition-colors border-white/20 outline-none ring-0 bg-white/10 text-white hover:bg-white/20 hover:text-white focus:text-white dark:bg-[#f8d24e]/10 dark:hover:bg-[#f8d24e]/20 dark:border-[#f8d24e]/20 dark:text-[#ffe28a] dark:hover:text-[#ffe28a] dark:focus:text-[#ffe28a] data-[state=open]:bg-white/20 dark:data-[state=open]:bg-[#f8d24e]/20 data-[state=open]:ring-2 data-[state=open]:ring-white/50 dark:data-[state=open]:ring-[#f8d24e]/50 data-[state=open]:ring-offset-0 data-[state=open]:text-white dark:data-[state=open]:text-[#ffe28a]")} aria-label={dropdownOpen ? "Close menu" : "Open menu"}>
                                <span className="relative flex size-5 items-center justify-center">
                                    <MenuIcon aria-hidden="true" className={cn(
                                        "absolute size-5 !text-white dark:!text-[#ffe28a] transition-all duration-200",
                                        dropdownOpen
