...
function ConnectionOptionButton({ title, description, icon, onClick, disabled = false }) {
    return (
        <Button variant="outline" className="group/button w-full h-auto justify-start gap-3 px-4 py-3 text-left mb-3 whitespace-normal" onClick={onClick} disabled={disabled}>
            <div className="bg-muted text-accent-foreground group-hover/button:bg-background rounded-md flex size-10 items-center justify-center shrink-0">
                {icon}
            </div>
...
export default function MfaSetupModal({ isOpen, email, onClose, onSaved }) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={() => {}}>
            <DialogContent onPointerDownOutside={(e) => e.preventDefault()} onInteractOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()} className="p-4 sm:max-w-[425px] gap-0 bg-white dark:bg-slate-950 border-none ring-0 outline-none shadow-xl [&>button]:hidden">
                ...
                <div className="py-2">
                        ...
                        <div>
                            ...
                            <div className="mb-4">
                                ...
                                <Field className="w-full">
                                    <InputGroup className="h-10 gap-0 border border-slate-300 dark:border-slate-700 rounded-md bg-white dark:bg-slate-950 transition-colors duration-200 !ring-0 focus-within:!ring-2 focus-within:!ring-slate-300 dark:focus-within:!ring-slate-600 focus-within:!border-slate-300 dark:focus-within:!border-slate-700 shadow-none overflow-hidden">
                            ...
                            <div className="mb-6 flex flex-col items-center">
                                ...
                                >
                                    <InputOTPGroup>
                                        <InputOTPSlot index={0} className={`w-10 h-12 text-lg sm:w-14 sm:h-16 sm:text-2xl ${errorMessage ? "border-red-500" : "border-slate-300 dark:border-slate-700"}`} />
                                        <InputOTPSlot index={1} className={`w-10 h-12 text-lg sm:w-14 sm:h-16 sm:text-2xl ${errorMessage ? "border-red-500" : "border-slate-300 dark:border-slate-700"}`} />
                                        <InputOTPSlot index={2} className={`w-10 h-12 text-lg sm:w-14 sm:h-16 sm:text-2xl ${errorMessage ? "border-red-500" : "border-slate-300 dark:border-slate-700"}`} />
                                    </InputOTPGroup>
                                    <InputOTPSeparator />
                                    <InputOTPGroup>
                                        <InputOTPSlot index={3} className={`w-10 h-12 text-lg sm:w-14 sm:h-16 sm:text-2xl ${errorMessage ? "border-red-500" : "border-slate-300 dark:border-slate-700"}`} />
                                        <InputOTPSlot index={4} className={`w-10 h-12 text-lg sm:w-14 sm:h-16 sm:text-2xl ${errorMessage ? "border-red-500" : "border-slate-300 dark:border-slate-700"}`} />
                                        <InputOTPSlot index={5} className={`w-10 h-12 text-lg sm:w-14 sm:h-16 sm:text-2xl ${errorMessage ? "border-red-500" : "border-slate-300 dark:border-slate-700"}`} />
                                    </InputOTPGroup>
                                </InputOTP>
                            </div>
                        </div>
                    ...
                    {step === "backupCodes" && (
                        <div>
                            ...
                            <div className="mb-6 rounded-xl border border-slate-200 dark:border-slate-800 bg-white shadow-sm dark:bg-slate-950 p-4">
                ...
                <div className="-mx-4 -mb-4 mt-2 flex flex-row items-center justify-end gap-2 rounded-b-xl border-t-0 bg-slate-50 dark:bg-slate-900/50 p-4">
                    {step === "verify" ? (
                        <Button variant="outline" onClick={() => setStep("scan")} className="rounded-lg h-8 px-4 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 hover:bg-slate-200 dark:hover:bg-slate-700 bg-white dark:bg-slate-900 font-bold text-sm">
                            Back
                        </Button>
                    ) : (
                        <Button variant="outline" onClick={handleClose} className="rounded-lg h-8 px-4 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 hover:bg-slate-200 dark:hover:bg-slate-700 bg-white dark:bg-slate-900 font-bold text-sm">
                            Cancel
                        </Button>
                    )}
                    
                    {step === "scan" && (
                        <Button onClick={() => setStep("verify")} disabled={isLoadingSetup || !setup.secret} className="rounded-lg h-8 px-4 bg-[#7b0d15] hover:bg-yellow-400 text-white hover:text-[#7b0d15] border-none font-bold text-sm transition-colors">
                            Next
                        </Button>
                    )}

                    {step === "verify" && (
                        <Button onClick={handleSave} disabled={isSaving} className="rounded-lg h-8 px-4 bg-[#7b0d15] hover:bg-yellow-400 text-white hover:text-[#7b0d15] border-none font-bold text-sm transition-colors">
                            {isSaving ? "Saving..." : "Save Authenticator"}
                        </Button>
                    )}

                    {step === "backupCodes" && (
                        <Button onClick={handleFinish} disabled={!hasCopiedBackupCodes} className="rounded-lg h-8 px-4 bg-[#7b0d15] hover:bg-yellow-400 text-white hover:text-[#7b0d15] border-none font-bold text-sm transition-colors">
                            Continue
                        </Button>
                    )}
                </div>
